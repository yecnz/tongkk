...
- 자료에 '# 이미지/손글씨 분석 결과' 섹션이 있으면 그 그림·그래프·도식의 내용(축·수치·관계·구조·단계)도 본문과 동등한 출제 대상으로 삼는다.
- 단, 비전 결과에 [불확실]/[확인 필요]로 표시된 값은 정답의 근거로 쓰지 않는다.
...
def _citation_rule(source_names: list[str] | None) -> str:
    ...
    base = (
        "15. 일반 요약과 강의 노트에서는 핵심 bullet이나 중요한 문장 끝에 가능한 경우 짧은 출처 표기를 붙여라. "
...
def _quiz_user_focus_section(focus_prompt: str | None) -> str:
    ...
    return (
        ...
        "- 집중 요청은 '무엇을 낼지' 고르는 데만 쓰고, 요청 문구나 '사용자가 집중·요청했다'는 사실 자체는 "
        "문제 지문·선택지·해설에 절대 드러내지 마라. 각 문항은 집중 요청을 모르는 사람도 강의 내용만으로 풀리도록 자연스럽게 출제해라.\n"
