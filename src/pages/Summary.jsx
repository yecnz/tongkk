          ...
          return (
            <Card key={m.key} style={{ padding: 0, overflow: "hidden", opacity: m.coming ? 0.45 : canAfford ? 1 : 0.5 }}>
              ...
              <div style={{ display: "flex", borderTop: "1px solid #f0f0f0" }}>
                ...
                }}>{m.coming ? "준비 중" : "선택"}</button>
                <button style={{
                  flex: 1, padding: "14px 0", border: "none",
                  background: "#fff", fontSize: 14, fontWeight: 600, color: "#888", cursor: m.coming ? "default" : "pointer"
                }}>공유</button>
              </div>
            </Card>
          );
...
const modelBadgeStyle = {
  Gemini: { background: "#FFF0F6", color: PINK },
  GPT:    { background: "#E8FAFE", color: CYAN },
  Claude: { background: "#F0F0FF", color: "#7C3AED" },
  Gemma4: { background: "#F0FFF4", color: "#16A34A" },
};

/* ── 요약 결과 뷰 ── */
const SummaryResultView = ({ modelKey, onBack, realContent, isLoading, error }) => {
  const data = summaryData[modelKey] || summaryData["Gemini"];
  const isSample = !realContent && !isLoading && !error;
  const displayContent = realContent || data.content;
  const badge = modelBadgeStyle[modelKey] || modelBadgeStyle["Gemini"];
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          ...
          {isSample && (
            <span style={{
              ...
              background: "#FFF8E1", color: "#F59E0B", border: "1px solid #FDE68A"
            }}>(샘플)</span>
          )}
