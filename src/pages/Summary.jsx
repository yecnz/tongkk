  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {models.map(m => {
          const canAfford = m.free || tokens >= m.cost;
          return (
            <Card key={m.key} style={{ padding: 0, overflow: "hidden", opacity: canAfford ? 1 : 0.5 }}>
              <div style={{ padding: "28px 20px", textAlign: "center" }}>
                <h3 style={{ margin: "0 0 20px", fontSize: 18, fontWeight: 700, color: "#222" }}>{m.name}</h3>
                <div style={{
                  background: "#fafafa", borderRadius: 14, padding: "32px 16px", marginBottom: 0,
                  minHeight: 100, display: "flex", alignItems: "center", justifyContent: "center"
                }}>
                  <span style={{ fontSize: 16, fontWeight: 600, color: "#555", whiteSpace: "pre-line", textAlign: "center", lineHeight: 1.6 }}>
                    {m.desc}
                  </span>
                </div>
              </div>
