                    ...
                    {courses.map((c, i) => (
                      <div key={i} style={{
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          {["요약", "퀴즈", "커뮤니티"].map(btn => {
                            const onClick = () => {
                              if (btn === "요약") navigate("/summary", { state: { course: c } });
                              if (btn === "퀴즈") navigate("/quiz", { state: { course: c } });
                              if (btn === "커뮤니티") navigate("/community", { state: { course: c } });
                            };

                            return (
                              <button key={btn} onClick={onClick} style={{
